...
OutputType main(ConstantOutputType input, float3 uvwCoord : SV_DomainLocation, const OutputPatch<InputType, 4> patch)
{
 ...
 //   cosVertexPosition.z += radialVector.z * cosSev;
    ...
    //cosVertexPosition.x = (cosVertexPosition.x* cos(20)) - (cosVertexPosition.y * sin(20));
    //cosVertexPosition.y = (cosVertexPosition.x* sin(20)) + (cosVertexPosition.y * cos(20));
    //cosVertexPosition.z = cosVertexPosition.z;



	float3 animVector = (4, 4, 4) * lerpAmount;



	cosVertexPosition.x = animVector.x + cosVertexPosition.x;
	cosVertexPosition.y = animVector.y + cosVertexPosition.y;
	cosVertexPosition.z = animVector.z + cosVertexPosition.z;
    ...
    vertexPosition += vertexPosition + (severity * noise(resolutionVector.x, resolutionVector.y, resolutionVector.z) * vertexPosition);
	...
	// Send the input color into the pixel shader.
	//colourModifier.x = (baseColour.x + saturate(tone));
	//colourModifier.y = (baseColour.y + saturate(tone));
	//colourModifier.z = (baseColour.z + saturate(tone));
	//colourModifier.w = 1.0f;
