	...
	Light* light, XMFLOAT3 cameraPos, int regionNum, XMFLOAT3 terrainBaseColor, bool identify, int numOfRegions)
{
	...
	lightPtr->regionNum = regionNum;
	lightPtr->terrainBaseColor = terrainBaseColor;
	lightPtr->identify = identify;
	lightPtr->numOfRegions = numOfRegions;
	lightPtr->padding = XMFLOAT2(0.0f, 0.0f);
...
void SpecularTerrainShader::Render(ID3D11DeviceContext* deviceContext, int indexCount)
{
