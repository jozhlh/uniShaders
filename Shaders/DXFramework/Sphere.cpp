...
Sphere::Sphere(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WCHAR* textureFilename, int resolution)
{
...
void Sphere::InitBuffers(ID3D11Device* device)
{
	...
	// Set the resolution
	float latitude_intervals = m_resolution;
	float longitude_intervals = m_resolution;
	...
	m_vertexCount = (latitude_intervals * longitude_intervals);
	...
	for (int longitude = 0; longitude < longitude_intervals; longitude++)
	{
		...
		for (int latitude = 0; latitude < latitude_intervals; latitude++)
		{
			...
			while (iterator < 4)
			{
				...
				vertices[v].texture = XMFLOAT2(tX, tY);
				...
				vertices[v].position = temp;

				// Normailse temp
				temp = Normalise(temp);

				vertices[v].normal = temp;
...
XMFLOAT3 Sphere::Calculate_Coordinates(float t, float d)
{
	float x = (radius * cosf(t)) * (sinf(d));
	float y = (radius * cosf(d));
	float z = (radius * sinf(t)) * sinf(d);

	return XMFLOAT3(x, y, z);
}

XMFLOAT3 Sphere::Normalise(XMFLOAT3 vec)
{
	float mag = sqrt((vec.x * vec.x) + (vec.y * vec.y) + (vec.z * vec.z));

	return XMFLOAT3(vec.x / mag, vec.y / mag, vec.z / mag);
}