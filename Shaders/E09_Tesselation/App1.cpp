...
void App1::init(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight, Input *in)
{
	...
	// Create Mesh object
	m_Mesh = new TessellationMesh(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), L"../res/checkerboard.png");
...
bool App1::Render()
{
	...
	*/
	...
	m_Mesh->SendData(m_Direct3D->GetDeviceContext());
	//// Set shader parameters (matrices and texture)
	m_TessellationShader->SetShaderParameters(m_Direct3D->GetDeviceContext(), worldMatrix, viewMatrix, projectionMatrix, m_Mesh->GetTexture());
	//// Render object (combination of mesh geometry and shader process
	m_TessellationShader->Render(m_Direct3D->GetDeviceContext(), m_Mesh->GetIndexCount());
